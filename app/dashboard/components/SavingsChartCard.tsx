...
export default function SavingsChartCard({ income, expenses }: SavingsChartCardProps) {
  ...
  return (
    <Card className="rounded-2xl border-zinc-800 bg-gradient-to-b from-zinc-950 to-zinc-900/80 text-zinc-100 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
      <CardHeader className="pb-4">
        <CardTitle className="text-2xl font-semibold text-zinc-100">Monthly Savings</CardTitle>
        <CardDescription className="mt-1 text-zinc-400">
          Income - total expenses for the selected month.
        </CardDescription>
        <p className="mx-auto mt-3 w-full rounded-md border border-zinc-700 bg-zinc-900/70 px-3 py-1 text-center text-sm font-semibold text-zinc-100 sm:max-w-[70%]">
          {savings >= 0 ? "Saved" : "Deficit"}: {formatAmount(Math.abs(savings))}
          {savingsPercent !== null
      ...
      <CardContent>
        <div className="flex h-[280px] items-center justify-center rounded-xl border border-zinc-800/80 bg-zinc-950/60 p-3">
          ...
          >
            <BarChart data={barChartData} margin={{ top: 18, right: 10, left: 10, bottom: 8 }}>
